...
public:
        ...
        */
        ...
        for(auto it = temp.begin();it != temp.end();++it) {
            if(it->second > HIT) {
                ...
                if(LDOS_IP.count(it->first.item)) {
                    real += it->second;
                }
                whole += it->second;
            }
        ...
        rr = (sketch->NAME == "HyperGSU") ? real / LDOS_TRAFFIC : real / (LDOS_TRAFFIC * FREQ_THRESHOLD);
    ...
    void TopKError(double alpha) {
        AbsVector FPIs = {
            new HyperGSU<3>(chunk * 20,0.2),
            new PeriodicSketch<3>(chunk * 200,0.2),
            //new Baseline(chuck * 30)
        };
...
private:
    ...
    HashMap mp;
    const char* PATH{};
    constexpr static COUNT_TYPE LDOS_TRAFFIC = 24988;
    std::unordered_set<uint64_t> LDOS_IP = {7734005253150081848ULL,7370469466171674665ULL,5295074171076183393ULL,1756382746195728506ULL\
    ,6509181133946966932ULL,579310637719607565ULL,12702417969235308750ULL,12272376654970843205ULL,3852791853557072090ULL,1667076546024897475ULL};

#ifndef PERIODICSKETCH_H
#define PERIODICSKETCH_H

#include "../HyperGSU/Sketch.h"
#define CELLNUM 8

template <uint32_t HASH_NUM>
class PeriodicSketch : public Sketch{
public:
    ...
    PeriodicSketch(uint32_t _MEMORY, double ratio = 0.15, std::string _name = "PeriodicSketch"){
        NAME = _name;
        MEMORY = _MEMORY;
    ...
    void Insert(const ItemPair& item){
        ...
        ItemPair temp(item.item,item.time - minTime);
        if(temp.time >= 1000)
            return; 
        ...
        for(uint32_t i = 0;i < HEAVY_LENGTH;++i){
            for(uint32_t j = 0;j < CELLNUM;++j){
                ...
                if(buckets[i].cells[j].count > HIT){
                    ret[ItemPair(buckets[i].cells[j].item,
                            buckets[i].cells[j].interval)]
